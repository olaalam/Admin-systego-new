...
const Paymob = () => {
    ...
    // جلب البيانات
    const { data, loading } = useGet(`/api/admin/paymob?refresh=${refreshKey}`);
    const paymobData = data?.paymob || [];

    // دالة الحذف
    // const handleDelete = async (id) => {
    //     if (window.confirm(t("Are you sure you want to delete this configuration?"))) {
    //         try {
    //             await api.delete(`/api/admin/paymob/${id}`);
    ...
    if (loading && paymobData.length === 0) return <Loader />;
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">{t("Paymob Settings")}</h1>
                    <p className="text-sm text-gray-500 mt-1">{t("Manage your payment gateway configurations")}</p>
                </div>
                <button
                    onClick={() => navigate("/paymob/add")}
            ...
            {paymobData.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-500 mt-2">{t("Click the Add Config button to create your first Paymob setup.")}</p>
                </div>
            ) : (
                    ...
                    {paymobData.map((item) => (
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-gray-800 text-lg">
                                            {item.payment_method_id?.name || t("Paymob")}
                                        </h3>
                                        <p className="text-xs text-gray-500 font-medium">
                                            {item.type?.toUpperCase() || "LIVE"}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div className="p-5 flex-1 bg-gray-50/50">
                                <div className="space-y-3 text-sm">
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                                        <span className="text-gray-500">{t("Integration ID")}</span>
                                        <span className="font-semibold text-gray-800">{item.integration_id || "-"}</span>
                                    </div>
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                                        <span className="text-gray-500">{t("Iframe ID")}</span>
                                        <span className="font-semibold text-gray-800">{item.iframe_id || "-"}</span>
                                    </div>
                                    {/* يمكن إضافة المزيد من التفاصيل هنا إذا رغبت */}
                                </div>
                            </div>
                            ...
                            <div className="p-4 bg-white flex items-center justify-between gap-3 border-t border-gray-50">
                                <button
                                    onClick={() => navigate(`/paymob/edit/${item._id}`)}
