          ...
          <div className="space-y-6 pt-4">
            {returns.map((ret) => (
              <div key={ret._id} className="border rounded-xl p-4 bg-slate-50 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-gray-600 text-white px-3 py-1 text-[10px] rounded-bl-lg font-bold">
                  {ret.reference}
                </div>
                  ...
                  {ret.items?.map((item, idx) => (
                    <div key={idx} className="p-3 flex items-center justify-between text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-bold text-gray-600">{item.subtotal} EGP</p>
                      </div>
                    </div>
                  ))}
                </div>
