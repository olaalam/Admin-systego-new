...
const PurchaseEdit = () => {
  ...
  useEffect(() => {
    ...
    if (p) {
      setFormData({
        ...
        // تصحيح مابينج المنتجات لجلب الضريبة والخصم والتاريخ لكل منتج
        purchase_items: (p.items || []).map(item => ({
          product_id: item.product_id?._id || item.product_id,
          name: item.product_id?.name || item.name || "Product",
          quantity: Number(item.quantity) || 0,
          unit_cost: Number(item.unit_cost) || 0,
          tax: Number(item.tax) || 0, // الضريبة لكل قطعة
          discount: Number(item.discount || item.discount_share) || 0, // الخصم لكل قطعة
        ...
        purchase_due_payment: (p.duePayments || []).map(dp => ({
  ...
  const handleUpdate = async () => {
    const payload = {
      ...
      purchase_items: totals.processedItems.map(({ name, subtotal, exp_ability, ...rest }) => {
        const item = { ...rest };
    ...
    } catch (err) { toast.error(t("Update Failed")); }
  };
