...
const Fawry = () => {
    ...
    // جلب البيانات
    const { data, loading } = useGet(`/api/admin/fawry?refresh=${refreshKey}`);

    // تعديل مسار قراءة الـ config ليتناسب مع الريسبونس الجديد
    const rawConfig = data?.data?.fawry || data?.fawry;
    const fawryData = rawConfig
    ...
    //         try {
    //             await api.delete(`/api/admin/fawry/${id}`);
    ...
    if (loading && fawryData.length === 0) return <Loader />;
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">{t("Fawry Settings")}</h1>
                    <p className="text-sm text-gray-500 mt-1">{t("Manage your payment gateway configurations")}</p>
                </div>
                <button
                    onClick={() => navigate("/fawry/add")}
            ...
            {fawryData.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-500 mt-2">{t("Click the Add Config button to create your first fawry setup.")}</p>
                </div>
            ) : (
                    ...
                    {fawryData.map((item) => (
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-gray-800 text-lg">
                                            {item.payment_method_id?.name || t("fawry")}
                                        </h3>
                            ...
                            <div className="p-5 flex-1 bg-gray-50/50">
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                                        ...
                                        <span className="font-semibold text-gray-800">{item.merchantCode || "-"}</span>
                                    </div>
                                    {/* تعديل الـ Iframe ID إلى Secure Key */}
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                                        <span className="text-gray-500">{t("Secure Key")}</span>
                                        <span className="font-semibold text-gray-800 truncate max-w-[120px]" title={item.secureKey}>{item.secureKey || "-"}</span>
                                    </div>
                                    {/* <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                            ...
                            <div className="p-4 bg-white flex items-center justify-between gap-3 border-t border-gray-50">
                                <button
                                    onClick={() => navigate(`/fawry/edit/${item._id}`)}
